module Environment (Environment, insert, replace, resolve, create, parent) where
...
data Environment a = Environment (Scope a) (Maybe (Environment a)) deriving (Show)

create :: [(String, a)] -> Maybe (Environment a) -> Environment a
create scope maybeParent = Environment (Map.fromList scope) maybeParent
...
resolve name env@(Environment scope maybeParent) =
    recoverWith (resolveLocal name env) fallback
